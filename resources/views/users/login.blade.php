...
<head>
    ...
    <title>User Login – {{ config('app.name', 'UNA Navigator') }}</title>
        ...
        <div class="relative z-10 w-full max-w-5xl grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div class="hidden md:flex flex-col bg-white/5 border border-white/10 backdrop-blur-lg rounded-2xl p-7 shadow-xl">
                ...
                <p class="text-sm text-slate-200 leading-relaxed mb-4">
                    Find buildings, rooms and services faster. Login to access interactive maps,
                    filters and accessibility information across campus.
                </p>
                ...
                <ul class="mt-1 space-y-2 text-[13px] text-slate-200/90">
                    ...
                    <li class="flex items-center gap-2">
                        ...
                        Interactive building & floor maps
                    </li>
                    <li class="flex items-center gap-2">
                ...
                <div class="mt-auto pt-4 text-[11px] text-slate-400">
                    New to UNA Navigator? Create an account to get started exploring campus.
                </div>
            </div>
            ...
            <div class="bg-white/5 border border-white/10 backdrop-blur-lg rounded-2xl p-7 sm:p-8 shadow-xl">
                <header class="mb-6">
                    <h1 class="text-xl font-semibold text-slate-50">User Login</h1>
                ...
                <p class="mt-5 text-[11px] text-center text-slate-300">
                    ...
                    <a href="{{ url('/users/register') }}" class="font-medium text-unaPrimary hover:text-cyan-300">
                        Create one
                    </a>
                </p>
            </div>
        ...
        document.getElementById('loginFormElement').addEventListener('submit', async function(e) {
            ...
            try {
                ...
                if (result.status === 'login_ok') {
                    // 用户登录成功，跳转到用户主页
                    let target = '/users/home';
                    
                    if (result.role === 'user') {
                        target = '/users/home';
                    } else {
                        showMessage('Please use admin login page.', 'error');
                        return;
                    }
