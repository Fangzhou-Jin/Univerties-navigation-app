...
<head>
    ...
    <style>
        ...
        /* Background glow can be softer in light mode */
        body.light-mode .pointer-events-none > div {
            opacity: 0.45;
        }
        ...
        /* Header "Back to Home" button (top right) */
        body.light-mode a[href*="/admins/home"] {
            background-color: #e5e7eb !important;   /* slate-200 */
            color: #111827 !important;              /* slate-900 */
            border-color: #cbd5e1 !important;
        }
        body.light-mode a[href*="/admins/home"]:hover {
            background-color: #d1d5db !important;   /* slate-300 */
        }
        ...
        body.light-mode .danger-zone-card h3 {
            color: #b91c1c !important;              /* red-700 */
        }

        body.light-mode .danger-zone-card p {
            color: #7f1d1d !important;              /* red-800 */
        }

        /* Danger Zone button stronger in light */
        body.light-mode .danger-zone-card button {
            background-color: #ef4444 !important;   /* red-500 */
            border-color: #b91c1c !important;       /* red-700 */
            color: #ffffff !important;
        }

        body.light-mode .danger-zone-card button:hover {
...
<body class="bg-unaBgDark text-slate-50 min-h-screen">
    ...
    <div class="relative z-10 min-h-screen p-6">
        <div class="max-w-4xl mx-auto">
            ...
            <div class="bg-white/5 backdrop-blur-md rounded-xl border border-white/10 p-6 mb-6">
                <div class="flex items-center gap-4 mb-6">
                    <div class="w-20 h-20 rounded-full bg-unaPrimary flex items-center justify-center text-2xl font-bold">
                        {{ strtoupper(substr($user->username_una ?? 'A', 0, 1)) }}
                    </div>
                    <div>
                        <h2 class="text-xl font-bold">{{ $user->username_una ?? 'Administrator' }}</h2>
                        <p class="text-sm text-slate-400">{{ $user->role->role_name_una ?? 'System Administrator' }}</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <div class="px-4 py-2 bg-white/5 border border-white/10 rounded-lg">
                            <span class="text-unaPrimary font-semibold">{{ $user->role->role_name_una ?? 'Administrator' }}</span>
                        </div>
                    </div>
            ...
            <div class="bg-white/5 backdrop-blur-md rounded-xl border border-white/10 p-6 mb-6">
                ...
                <form id="editProfileForm" class="space-y-4" action="{{ route('admins.profile.update') }}" method="POST">
                    @csrf
                    <div>
            ...
            <!-- Danger Zone Card -->
            <div class="danger-zone-card bg-red-900/20 backdrop-blur-md rounded-xl border border-red-500/30 p-6">
    ...
    <script>
        ...
        document.getElementById('editProfileForm').addEventListener('submit', function(e) {
            ...
            // Send to backend
            fetch('{{ route("admins.profile.update") }}', {
                method: 'POST',
                body: formData,
                headers: {
    ...
    <!-- THEME SYNC (use una-dark from other pages) -->
    <script>
