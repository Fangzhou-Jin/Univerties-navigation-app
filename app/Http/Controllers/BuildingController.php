...
class BuildingController extends Controller
{
    ...
    {
        // 带上所属大学信息，方便前端在 Building 名称后面追加大学名
        $buildings = Building::with('university')
            ->where('id_university_una', $universityId)
            ->get();
        return response()->json($buildings);
    }
    ...
    /**
     * Get all unique floors (independent of building)
     */
    public function getAllFloors()
    {
        $floors = Room::select('floor_number_una')
            ->distinct()
            ->orderBy('floor_number_una')
            ->get()
            ->pluck('floor_number_una')
