...
class RoomController extends Controller
{
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'directions_una' => 'required|string|max:255',
        ]);
    ...
    public function search(Request $request)
    {
        ...
        // Support both single and multiple values for university_id
        if ($request->has('university_id')) {
            $universityIds = is_array($request->university_id) ? $request->university_id : [$request->university_id];
            if (!empty($universityIds)) {
                $query->whereIn('id_university_una', $universityIds);
            }
        }

        // Support both single and multiple values for building_id
        if ($request->has('building_id')) {
            $buildingIds = is_array($request->building_id) ? $request->building_id : [$request->building_id];
            if (!empty($buildingIds)) {
                $query->whereIn('id_building_una', $buildingIds);
            }
        }

        // Support both single and multiple values for floor_number
        if ($request->has('floor_number')) {
            $floorNumbers = is_array($request->floor_number) ? $request->floor_number : [$request->floor_number];
            if (!empty($floorNumbers)) {
                $query->whereIn('floor_number_una', $floorNumbers);
            }
        }
